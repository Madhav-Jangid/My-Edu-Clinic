...
import {
  ApproveOrganizationasPerid,
  rejectOrganization,
} from "@/lib/database/actions/expert.action";
...
const OrgCard = ({ orgData }: orgCardProps) => {
  ...
  return (
    <div className="min-h-[300px] w-full px-3 bg-white shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] rounded-md border-[1px] border-zinc-300 py-3 flex flex-col lg:flex-row gap-5">
  <div className="h-[220px] lg:w-4/12 bg-slate-200 rounded-md">
    <Image
  ...
  <div className="w-full lg:w-8/12 px-4">
    <h1 className="text-2xl font-semibold mt-1 text-zinc-700">
      {orgData.orgName}
    </h1>
    ...
    <div className=" flex gap-2 items-center mt-3">
      ...
      <p className="text-sm font-medium">
        Internship and Placement Service
      </p>
    </div>
    <div className=" flex gap-2 items-center mt-3">
      <GitCompare className=" text-green-700" size={18} strokeWidth={1.5} />
    ...
    <p className="text-[12px] font-normal mt-4 text-zinc-500">
    ...
    <div className="flex flex-col lg:flex-row justify-between mt-4">
      <Button
        ...
        className="bg-indigo-700 lg:w-auto w-full"
      >
        Approve Organization
      </Button>
      <Dialog>
        <DialogTrigger className="w-full font-medium text-zinc-800 border-[1px] border-zinc-400 text-sm rounded-md lg:w-auto mt-4 bg-red-500 p-2 lg:mt-0">
          Decline Approval
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              <Textarea
                ...
                className="mt-4"
                placeholder="Please describe the reason for declination..."
              />
              <Button
                onClick={() => {
